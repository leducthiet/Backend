...
public interface TourBookingRepository extends JpaRepository<TourBooking, Long> {
    ...
    @Query(value = "SELECT tb.* FROM tour_booking tb JOIN tour_date_booking td ON td.id = tb.tour_date_booking_id JOIN tour t ON t.id = td.tour_id WHERE t.travel_agency_id = ?1", nativeQuery = true)
    List<TourBooking> getTourBookingByTravelAgencyId(Long travel_agency_id);

    @Query(value = "SELECT tb.* FROM tour_booking tb where tb.tour_date_booking_id = ?1", nativeQuery = true)
    List<TourBooking> getTourBookingByTourDateBookingId(Long tourDateBookingId);
    ...
    @Query(value = "SELECT ISNULL(SUM(tb.total_price), 0) FROM tour_booking tb JOIN users u ON u.id = tb.user_id WHERE tb.booking_state = 2 AND MONTH(tb.date_create) = ?1 AND u.travel_agency_id = ?2", nativeQuery = true)
    Long getIncomeByMonth(int month, Long travelAgencyId);
